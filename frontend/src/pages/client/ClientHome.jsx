  ...
  return (
    <div>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-3 gap-4 mb-8">
        ...
        <div className="bg-gradient-to-br from-green-400 to-green-600 rounded-2xl p-6 text-white shadow-md relative">
          <p className="text-green-100 text-sm mb-1">Notifications</p>
          <p className="text-4xl font-black">{compteurNotif}</p>
          {compteurNotif > 0 && (
            <span className="absolute top-4 right-4 w-3 h-3 bg-white rounded-full animate-ping"></span>
          )}
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 mb-8">
        <Link to="/client/deposer" className="bg-white border-2 border-dashed border-secondary rounded-2xl p-6 hover:bg-blue-50 transition group flex items-center gap-4">
        ...
        <Link to="/client/boutique" className="bg-white border-2 border-dashed border-accent rounded-2xl p-6 hover:bg-orange-50 transition group flex items-center gap-4">
      ...
      <div className="bg-white rounded-2xl shadow-sm border border-gray-100 p-6">
        <div className="flex justify-between items-center mb-6">
          ...
          <Link to="/client/commandes" className="text-secondary text-sm font-medium hover:underline">
            Voir tout →
          </Link>
        </div>
        ...
        {loading ? (
          ...
          <div className="text-center py-10">
            ...
            <Link to="/client/deposer" className="inline-block mt-4 bg-gradient-to-r from-primary to-secondary text-white px-6 py-2.5 rounded-xl text-sm font-semibold hover:opacity-90 transition">
              Déposer un document
            </Link>
          </div>
        ) : (
          <div className="space-y-3">
            {commandesRecentes.map((commande) => (
              <div key={commande.idCommande} className="flex items-center gap-4 p-4 bg-gray-50 rounded-xl hover:bg-gray-100 transition">
                ...
                <span className={`text-xs px-3 py-1 rounded-full font-medium flex-shrink-0 ${getStatutStyle(commande.statut)}`}>
